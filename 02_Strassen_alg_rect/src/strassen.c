                                    ...
                                    float const *const *const B, size_t n, size_t m, size_t p) 
{
    ...
    // find the biggest dimension among n, m, p
    size_t max;
    if (n >= m){
        if(n >= p){
            max = n;
    ...
    // compute the closest power of 2 to max
    size_t bigsize;
    for(size_t i = 1; i < INT_MAX; i *= 2){
        if (max < i){
        bigsize = i;
        break;
        }
    ...
    // creating and padding the bigger matrices
    float **bigA = allocate_zero_matrix(bigsize, bigsize);
    float **bigB = allocate_zero_matrix(bigsize, bigsize);
    float **bigC = allocate_zero_matrix(bigsize, bigsize);
    ...
    strassen_aux((float**) bigC, (float const *const *const) bigA, (float const *const *const) bigB,
                    0, 0,
                    0, 0,
                    0, 0,
                    bigsize);

    // coping in C the non-zero part of bigC
    for (size_t i = 0; i < n; i++) {
        for (size_t j = 0; j < p; j++) {
            C[i][j] = bigC[i][j];
        }
                                    ...
                                    float const *const *const B, size_t n, size_t m, size_t p) 
{
    ...
    strassen_aux_best((float **) bigC, (float const *const *const) bigA, (float const *const *const) bigB,
                    0, 0,
                    0, 0,
                    0, 0,
                    bigsize);
