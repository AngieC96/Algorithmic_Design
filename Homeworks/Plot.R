...
n <- sep(0, 0, l = dim(T)[1])
heap <- sep(0, 0, l = dim(T)[1])
array <- sep(0, 0, l = dim(T)[1])
for(i in 1:dim(T)[1]) {
	n[i] <- T[i, 1]
	heap[i] <- T[i, 2]
	array[i] <- T[i, 3]
...
plot(n, heap, xlab = "Dimension", ylab = "Time [s]", main = "Plot ......", col = 3)
lines(n, heap, col = 4)
axis(1, at=x, labels=x)
legend(x = nx_min, y = maxylim, legend=c("Computational", "Elapsed"), col = c(3, 4), lty=1, cex=1.5)
dev.off()
